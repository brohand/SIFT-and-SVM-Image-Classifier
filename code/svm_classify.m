...
for i = 1:num_categories
    matching_labels = strcmp(categories(i),train_labels);
    binary_labels = ones(size(matching_labels)) .* -1;
    binary_labels(matching_labels) = 1;
    binary_labels = transpose(binary_labels);
    [w, b] = vl_svmtrain(single(train_image_feats)', binary_labels, lambda);
    W(i,:) = w;
    B(i,:) = b;
...
for i = 1:M
    ...
    for k = 1:num_categories
        confidence = dot(single(W(k,:)), single(test_image_feats(i,:))) + B(k,:);
