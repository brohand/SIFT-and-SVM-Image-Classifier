...
sample_image_count = size(image_paths,1);
...
for i = 1:sample_image_count
    filename = image_paths(i);
    filename = char(filename);
    
    Img = imread(filename);
    I = im2single(Img);
    [~,SIFT_feats] = vl_dsift(I, 'step', step, 'fast', 'size', bins);
    
    Image_SIFT_feats = horzcat(Image_SIFT_feats, SIFT_feats);
...
[centers, ~] = vl_kmeans(single(Image_SIFT_feats), vocab_size);
vocab = centers;
end
